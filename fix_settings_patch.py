...
def get_app_paths():
    ...
    優先順位:
    1. 環境変数 APP_DATA_DIR
    2. frozen バイナリ（PyInstaller / py2app）
    3. 開発環境（ソースコード実行）
    """
    # 1. 環境変数があれば最優先
    ...
    if env_data_dir:
        ...
        settings_file = data_dir / "crawled_data" / "settings.json"
        # 実行ファイルの所在はカレントディレクトリで代替
    ...
    project_root = bundle_dir  # ルートディレクトリをデータディレクトリとして使用
